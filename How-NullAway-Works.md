...
Work is underway to implement support for the [JSpecify](https://jspecify.dev) specification of the semantics of Java nullability annotations.  See the [JSpecify Support](JSpecify-Support) page for details from a user perspective; here we give more details on the implementation thus far.  You should be familiar with the [JSpecify user guide](https://jspecify.dev/docs/user-guide/) before reading this and you may need to reference [the full spec](https://jspecify.dev/docs/spec/).  Most code for implementing JSpecify support is in the [`com.uber.nullaway.generics`](https://github.com/uber/NullAway/tree/dd0da6d4628c36bed659f862cb5b5d0bd19cb263/nullaway/src/main/java/com/uber/nullaway/generics) package, specifically the [`GenericsChecks`](https://github.com/uber/NullAway/blob/dd0da6d4628c36bed659f862cb5b5d0bd19cb263/nullaway/src/main/java/com/uber/nullaway/generics/GenericsChecks.java) class.  

**Null-markedness**: NullAway supports the `@NullMarked` and `@NullUnmarked` annotations, including nesting them.  The core logic for checking the markedness of a particular method / class is in the [`CodeAnnotationInfo`](https://github.com/uber/NullAway/blob/dd0da6d4628c36bed659f862cb5b5d0bd19cb263/nullaway/src/main/java/com/uber/nullaway/CodeAnnotationInfo.java) class.  The main `NullAway` class calls into `CodeAnnotationInfo` methods as it traverses ASTs to determine what checks to perform.  It also tries to optimize for the common case where a class is entirely `@NullMarked` or `@NullUnmarked`; see the [`NullMarking` enum](https://github.com/uber/NullAway/blob/dd0da6d4628c36bed659f862cb5b5d0bd19cb263/nullaway/src/main/java/com/uber/nullaway/NullAway.java#L206) and how it is used.

**Ensuring type arguments respect bounds**: For a generic type with explicit nullability in type arguments (e.g., `Foo<@Nullable String>`), we must check that a type argument is `@Nullable` only if the corresponding type variable has a `@Nullable` upper bound (e.g., `class Foo<T extends @Nullable Object>`, but not just `class Foo<T>`).  See [`GenericsChecks.checkInstantiationForParameterizedTypedTree`](https://github.com/uber/NullAway/blob/ca24070039f81816c2657ea6090d9bbd8c106eea/nullaway/src/main/java/com/uber/nullaway/generics/GenericsChecks.java#L115-L115) and [`GenericsChecks.checkGenericMethodCallTypeArguments`](https://github.com/uber/NullAway/blob/ca24070039f81816c2657ea6090d9bbd8c106eea/nullaway/src/main/java/com/uber/nullaway/generics/GenericsChecks.java#L196-L196) for the relevant logic.

Note that getting the fully-annotated type of an expression within NullAway can be challenging (even ignoring inference), as sometimes `javac` maintains annotations inconsistently or not at all (particularly for `NewClassTree`s).  See the [`GenericsChecks.getTreeType`](https://github.com/uber/NullAway/blob/dd0da6d4628c36bed659f862cb5b5d0bd19cb263/nullaway/src/main/java/com/uber/nullaway/generics/GenericsChecks.java#L406-L406) method for our additional logic to get or reconstruct annotated types in more cases.  

**Determining when an expression is `@Nullable`** Some core NullAway logic for determining whether an expression or location is `@Nullable` must be enhanced for JSpecify.  E.g., a class `Foo<T>` may have an instance method `T getVal() { ... }`.  When `getVal()` is invoked with a receiver of type `Foo<@Nullable String>`, NullAway must reason that the invocation expression is `@Nullable`.  Similar cases arise when determining if a method parameter is `@Nullable` or not.  There are various methods in `GenericsChecks` for assisting with this reasoning, including [`getGenericReturnNullnessAtInvocation`](https://github.com/uber/NullAway/blob/ca24070039f81816c2657ea6090d9bbd8c106eea/nullaway/src/main/java/com/uber/nullaway/generics/GenericsChecks.java#L1076-L1076), [`getGenericMethodParameterNullness`](https://github.com/uber/NullAway/blob/ca24070039f81816c2657ea6090d9bbd8c106eea/nullaway/src/main/java/com/uber/nullaway/generics/GenericsChecks.java#L1365-L1365), and [`getGenericParameterNullnessAtInvocation`](https://github.com/uber/NullAway/blob/ca24070039f81816c2657ea6090d9bbd8c106eea/nullaway/src/main/java/com/uber/nullaway/generics/GenericsChecks.java#L1267-L1267).

**Subtyping checks**: NullAway must perform subtyping checks at (pseudo-)assignments and at method overrides.  With JSpecify support, these subtyping checks must also account for nullability of generic type arguments and array contents.  The code for the additional subtyping checks required by JSpecify is separated from the code checking subtyping of top-level nullability (i.e., that a `@Nullable` expression is not assigned into a `@NonNull` location), since JSpecify mode is still optional.  The main logic for the additional JSpecify subtyping checks are in [`GenericsChecks.subtypeParameterNullability`](https://github.com/uber/NullAway/blob/dd0da6d4628c36bed659f862cb5b5d0bd19cb263/nullaway/src/main/java/com/uber/nullaway/generics/GenericsChecks.java#L785-L785), which in turn calls [`GenericsChecks.identicalTypeParameterNullability`](https://github.com/uber/NullAway/blob/dd0da6d4628c36bed659f862cb5b5d0bd19cb263/nullaway/src/main/java/com/uber/nullaway/generics/GenericsChecks.java#L772-L772), which uses the [`CheckIdenticalNullabilityVisitor`](https://github.com/uber/NullAway/blob/dd0da6d4628c36bed659f862cb5b5d0bd19cb263/nullaway/src/main/java/com/uber/nullaway/generics/CheckIdenticalNullabilityVisitor.java).  These routines are called from various places to handle regular assignments, parameter passing, returns, etc.  For method overriding checks see [`GenericsChecks.checkTypeParameterNullnessForMethodOverriding`](https://github.com/uber/NullAway/blob/ca24070039f81816c2657ea6090d9bbd8c106eea/nullaway/src/main/java/com/uber/nullaway/generics/GenericsChecks.java#L940-L940).

Sometimes when performing a subtyping check, we need to "view" a type as one of its supertypes.  E.g., if we have `class Foo<T extends @Nullable Object>`, and `class Bar extends Foo<@Nullable String>`, we may have to check if a `Bar` expression is assignable to a `Foo<String>` variable (it is not).  To do so, we need to first "view" the `Bar` expression as `Foo<@Nullable String>` and then compare the nullability of the type arguments.  The `TypeSubstitutionUtils` class contains methods to help with such conversions (the [`asSuper`](https://github.com/uber/NullAway/blob/c187bf593f8a2979ca9770cb7849619f334daef4/nullaway/src/main/java/com/uber/nullaway/generics/TypeSubstitutionUtils.java#L35-L35) method), along with viewing a method as a member of some class ([`memberType`](https://github.com/uber/NullAway/blob/c187bf593f8a2979ca9770cb7849619f334daef4/nullaway/src/main/java/com/uber/nullaway/generics/TypeSubstitutionUtils.java#L63-L63)) and substituting type arguments into a generic type ([`subst`](https://github.com/uber/NullAway/blob/c187bf593f8a2979ca9770cb7849619f334daef4/nullaway/src/main/java/com/uber/nullaway/generics/TypeSubstitutionUtils.java#L283-L283)).  (The class is named `TypeSubstitutionUtils` since all of these operations require performing substitutions.)  The logic in this class handles cases where `@Nullable` type arguments are written in `extends` and `implements` clauses, with some additional related logic in [`ClassDeclarationNullnessAnnotUtils`](https://github.com/uber/NullAway/blob/c187bf593f8a2979ca9770cb7849619f334daef4/nullaway/src/main/java/com/uber/nullaway/generics/ClassDeclarationNullnessAnnotUtils.java#L26-L26).  It also handles restoring explicit nullability annotations (see [`restoreExplicitNullabilityAnnotations`](https://github.com/uber/NullAway/blob/c187bf593f8a2979ca9770cb7849619f334daef4/nullaway/src/main/java/com/uber/nullaway/generics/TypeSubstitutionUtils.java#L85-L85)).  This restoration handles cases like when a method parameter `p` has type `@NonNull T` where `T` is a type variable and the `@NonNull` annotation is explicit.  Even if the type argument for `T` is `@Nullable` at some call site, that parameter must still be treated as `@NonNull` due to the explicit annotation.

**Inference of Type Argument Nullability** For generic methods, Java often infers the type arguments for calls to such methods.  For JSpecify support, Nullaway must extend that inference to also infer nullability of the type arguments.  We have an initial implementation of inference based on a simple constraint solver.  See [`GenericsChecks.inferGenericMethodCallType`](https://github.com/uber/NullAway/blob/c187bf593f8a2979ca9770cb7849619f334daef4/nullaway/src/main/java/com/uber/nullaway/generics/GenericsChecks.java#L517-L517), [`ConstraintSolver`](https://github.com/uber/NullAway/blob/c187bf593f8a2979ca9770cb7849619f334daef4/nullaway/src/main/java/com/uber/nullaway/generics/ConstraintSolver.java#L12-L12), and [`ConstraintSolverImpl`](https://github.com/uber/NullAway/blob/c187bf593f8a2979ca9770cb7849619f334daef4/nullaway/src/main/java/com/uber/nullaway/generics/ConstraintSolverImpl.java#L32-L32).  The basic approach is to generic subtyping constraints for a call based on where its result is being assigned and what parameters are being passed, and then solve the constraints by propagating nullability requirements, while still respecting type variable bounds.  This code is still in flux and the implementation approach may change in the future.
