...
**Determining when an expression is `@Nullable`** Some core NullAway logic for determining whether an expression or location is `@Nullable` must be enhanced for JSpecify.  E.g., a class `Foo<T>` may have an instance method `T getVal() { ... }`.  When `getVal()` is invoked with a receiver of type `Foo<@Nullable String>`, NullAway must reason that the invocation expression is `@Nullable`.  Similar cases arise when determining if a method parameter is `@Nullable` or not.  There are various methods in `GenericsChecks` for assisting with this reasoning, including [`getGenericReturnNullnessAtInvocation`](https://github.com/uber/NullAway/blob/ca24070039f81816c2657ea6090d9bbd8c106eea/nullaway/src/main/java/com/uber/nullaway/generics/GenericsChecks.java#L1076-L1076), [`getGenericMethodParameterNullness`](https://github.com/uber/NullAway/blob/ca24070039f81816c2657ea6090d9bbd8c106eea/nullaway/src/main/java/com/uber/nullaway/generics/GenericsChecks.java#L1076-L1076), and [`getGenericParameterNullnessAtInvocation`](https://github.com/uber/NullAway/blob/ca24070039f81816c2657ea6090d9bbd8c106eea/nullaway/src/main/java/com/uber/nullaway/generics/GenericsChecks.java#L1267-L1267).
...
* key additional features
  * Support for `@NullMarked` and `@NullUnmarked`
  * Instantiating type variables when checking basic nullness in NullAway
  * checking type arguments are within bounds
  * enhanced subtyping check to account for generics, arrays, etc.
    * at pseudo-assignments
    * at overrides
    * viewing as supertype, explicit annotations in extends
